...
- Fable Advisor may explicitly use the dedicated Python API transport. It remains a root-facing read-only MCP operation, not a spawned Agent: URL/model mapping is operator-controlled, credentials live only in the dedicated local config, setup/status make no model call, and runtime never redirects, retries, or falls back to CC Switch, environment, Claude settings, or subscription auth.
- Designer may explicitly use its own dedicated Python API transport. It is a separate root-facing, stateless MCP operation with an independently configured provider, endpoint, authentication type, exact model, protocol, and token bound. Setup/status make no model call; runtime requires exact model echo, `end_turn`, and a non-empty `DESIGN_COMPLETE` handoff, and never redirects, retries, edits files, persists a session, or falls back to OpenRouter, Claude Code, CC Switch, environment credentials, or another provider.
