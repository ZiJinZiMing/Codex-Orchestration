...
class SkillContractTests(unittest.TestCase):
    ...
    def test_fable_is_a_bundled_root_only_mcp_advisor(self) -> None:
        ...
        self.assertIn("first-party Pro or Max Claude Code login", SKILL)
        self.assertIn("--advisor-fable-transport cc-switch-openrouter-loopback", SKILL)
        self.assertIn("fixed `127.0.0.1:15721` loopback", SKILL)
        self.assertIn("never copies or writes a token", SKILL)
        self.assertIn("runtime `modelUsage` to confirm `claude-fable-5`", SKILL)
        self.assertIn("Claude Code's `--json-schema` output contract", SKILL)
        self.assertIn("unique Claude Code session ID", SKILL)
        self.assertIn("SQLite `rowid` cursor", SKILL)
