...
Add `--advisor-model` and `--advisor-effort` for a same-provider Codex advisor. For Claude Fable 5, use `--advisor-fable --advisor-effort max --advisor-auth-mode auto --advisor-transport claude-code`. The authentication mode is `subscription`, `api`, or `auto`; the transport is `claude-code` or `direct-api`, with `claude-code` as the backward-compatible default. `direct-api` requires explicit `--advisor-auth-mode api --advisor-api-source environment|user-settings`. `auto` checks subscription only when no API credential is configured; when API configuration exists it fails closed and requires explicit API mode and source, preventing an accidental metered route. The configurator chooses an available Python 3.11+ MCP launcher and performs only a local auth/capability check during setup. It never writes a credential or makes a model call during setup or status. Omission persists `advisor: none`.
...
In every user-facing status or result, use the exact name `Claude Fable 5`. Report the transport as `claude-code` or `direct-api` and the active authentication path as `subscription` or `api`; do not expose or restate credentials, endpoints, or account identifiers, and do not expose or restate Claude account-plan metadata.
...
`direct-api` requires one static token or API key and rejects `apiKeyHelper`, custom headers, ambiguous credentials, unsafe URLs, redirects, and every subscription/auto combination. It sends exactly one dependency-free Python request with zero retries to the selected Anthropic-compatible Messages endpoint, uses a 65,536-token output cap and a 600-second request timeout, requires `anthropic-version: 2023-06-01`, and accepts only `stop_reason=end_turn`. Raw response model matching is byte-exact against the fixed allowlist `claude-fable-5` and `anthropic/claude-fable-5`; either value normalizes to canonical Fable-only metadata while `response_model` preserves the raw echo. It honors normal HTTP(S) proxy environment behavior. The saved effort is reported as `configured_effort` but is not sent or applied. The saved route pins the model and non-secret transport/authentication/source enums; the root cannot replace them through tool arguments.
