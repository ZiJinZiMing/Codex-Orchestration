...
| Python API Designer | Root-directed `create_design` reads its own dedicated config file and sends one Anthropic Messages request to the exact configured URL/model mapping | Designer-only explicit transport; requires exact model echo, `end_turn`, and a non-empty `DESIGN_COMPLETE` handoff, with no OpenRouter/Claude Code/CC Switch/environment fallback, redirects, retries, tools, edits, or persistence. |
...
Designer has an independent config-file-only Python API transport. Run
`configure_designer_api.py` to create
`~/.codex/.codex-orchestration-designer-api.json`, then select `--designer-api`.
The strict schema stores role `designer`, provider ID, full Anthropic Messages URL,
exact API model ID, bearer or `x-api-key` authentication, protocol, response-token
bound, and API key. The credential is accepted only through a hidden prompt or
`--api-key-stdin`, remains plaintext in the ACL-protected local file, and is
excluded from status and routing fingerprints. Setup/status validate configuration
identity without a model call. `create_design` sends exactly one non-redirecting,
non-retrying request and requires exact response-model echo, `end_turn`, a first
non-empty line of `DESIGN_COMPLETE`, and a non-empty body. Kimi Coding Plan uses
API model `k3`; its Claude Code alias `k3[1M]` is not a valid API model mapping.
This route has no OpenRouter role, Claude Code, CC Switch, environment, alternate
provider, or subscription fallback. It can coexist with either Fable Advisor
transport because its config and launcher family are separate.
...
Saved state compatibility is explicit: schema 1 must carry policy version 1 and predates Fable and Planner; schema 2 must carry policy version 2 and may authorize only the historical Fable Advisor shape; schema 3 must carry policy version 3 and adds Planner; schema 4 must carry policy version 4 and adds the optional direct-model Designer route plus the exact config-file Python API metadata on a Fable Advisor only; schema 5 must carry policy version 5 and adds the exact config-file Python API Designer route plus independent Fable and Designer launcher families. Schema and policy values must be actual JSON integers, not booleans or floats. Legacy state cannot contain fields introduced later; nested snapshots, scalar conversion, MCP launchers, and routes must match an emitted contract; and managed policy strings must carry the plugin marker before status, seat change, disable, or either bridge trusts them. Schemas 1–4 are read without rewrite and migrate only on explicit setup. Designer cannot use Fable or a persistent unqualified agent name, while Planner/Advisor independence remains the only route-separation rule. Unknown extensions intentionally fail closed. Disable with version 0.9 before downgrading to 0.8, whose older validator must reject schema 5 rather than guessing how to restore it.
