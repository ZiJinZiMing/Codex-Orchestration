...
Claude Fable 5 is the explicit built-in exception. The plugin does not pretend it is a Codex model or translate Anthropic into the Responses protocol. Instead, a disabled-by-default local MCP server invokes the official `claude` CLI through one explicit transport profile: the default first-party Pro/Max login, or the fixed local CC Switch/OpenRouter loopback when the user selects it. Setup enables one Python 3.11+ launcher variant, and disable restores every prior plugin override value. Codex's TOML editor can retain an inert empty table header after its final key is deleted; the configurator does not risk a broad TOML rewrite for cosmetic cleanup.

The bridge removes inherited `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`, `ANTHROPIC_BASE_URL`, and Bedrock/Vertex/Foundry selection variables from the child environment. Claude Code then loads only the user settings selected by `--setting-sources user`. The CC Switch/OpenRouter profile accepts only `http://127.0.0.1:15721`, bypasses HTTP proxies and redirects for its health check, verifies the current provider/mapping from the local database, and stores no provider settings or credentials in Codex state. It pins `claude-fable-5` and the saved effort, disables tools and session persistence, disables prompt suggestions, requires a JSON Schema-validated decision and review, generates a unique Claude Code session ID, and requires both JSON `modelUsage` and exactly one matching post-cursor CC Switch Fable request row to confirm the route. Claude Code may make non-Fable structured-output helper requests under that session; those are ignored rather than treated as advisor evidence. Setup and status never make a model call.
